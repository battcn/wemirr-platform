-- 模块菜单
INSERT INTO `sys_resource` (`name`, `title`, `permission`, `path`, `component`, `icon`, `sort`, `type`, `parent_id`,`platform_id`, `del_flag`, `create_time`, `update_time`)
SELECT  '${package.ModuleName}', '${package.ModuleName}', null, null, null, null, '100', '1', '1', ${cfg.platformId},'0', '${cfg.now}', '${cfg.now}'
FROM DUAL WHERE NOT EXISTS(SELECT id FROM sys_resource WHERE name = '${package.ModuleName}');

-- 查找模块菜单id
set @parentId = (select id from sys_resource where  name = '${package.ModuleName}');
