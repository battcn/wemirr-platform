package com.wemirr.platform.warmflow.domain.vo;
...
//    @Translation(type = FlowConstant.CATEGORY_ID_TO_NAME, mapper = "category")
    ...
    private void updateRunDuration() {
        ...
        if (this.updateTime != null && this.createTime != null) {
            long time = DateUtil.between(this.updateTime, this.createTime, DateUnit.HOUR);
            long day = TimeUnit.MILLISECONDS.toDays(time);
            long hour = TimeUnit.MILLISECONDS.toHours(time) % 24;
            long min = TimeUnit.MILLISECONDS.toMinutes(time) % 60;
            ...
            this.runDuration = !result.isEmpty() ? result.toString().trim() : "0秒";
        }
    }
